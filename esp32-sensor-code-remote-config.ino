...
void checkForConfigurationUpdates() {
  ...
  String jsonData = "{\"sensor_id\":\"" + String(SENSOR_ID) + "\",\"action\":\"get_config\"}";
...
void parseConfiguration(String jsonResponse) {
  ...
  if (newConfigVersion > config.config_version || hasPendingChanges) {
    ...
    if (doc.containsKey("sensors")) {
      ...
      // Update vijver_water sensor (index 0)
      if (sensors.containsKey("vijver_water")) {
        JsonObject vijverWater = sensors["vijver_water"];
        config.sensors[0].display_name = vijverWater["display_name"].as<String>();
        config.sensors[0].temperature_offset = vijverWater["temperature_offset"] | config.sensors[0].temperature_offset;
        config.sensors[0].temperature_scale = vijverWater["temperature_scale"] | config.sensors[0].temperature_scale;
        config.sensors[0].enabled = vijverWater["enabled"] | config.sensors[0].enabled;
      }
      
      // Update filter_inlaat sensor (index 1)
      if (sensors.containsKey("filter_inlaat")) {
        JsonObject filterInlaat = sensors["filter_inlaat"];
        config.sensors[1].display_name = filterInlaat["display_name"].as<String>();
        config.sensors[1].temperature_offset = filterInlaat["temperature_offset"] | config.sensors[1].temperature_offset;
        config.sensors[1].temperature_scale = filterInlaat["temperature_scale"] | config.sensors[1].temperature_scale;
        config.sensors[1].enabled = filterInlaat["enabled"] | config.sensors[1].enabled;
      }
    }
...
void markConfigurationApplied() {
  ...
  String jsonData = "{\"sensor_id\":\"" + String(SENSOR_ID) + "\",\"action\":\"mark_applied\"}";
...
void sendDataToSupabase(float temperature, String sensorType) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // JSON data with sensor type
    String jsonData = "{\"temperature\":" + String(temperature, 2) + ",\"sensor_id\":\"" + String(SENSOR_ID) + "\",\"device_id\":\"" + String(DEVICE_ID) + "\",\"sensor_type\":\"" + sensorType + "\"}";
...
void sendPingToSupabase() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // JSON data for ping
    String jsonData = "{\"sensor_id\":\"" + String(SENSOR_ID) + "\",\"device_id\":\"" + String(DEVICE_ID) + "\",\"ping_time\":" + String(millis()) + "}";
...
void initializeSensorConfigs() {
  // Initialize sensor 1 (index 0)
  config.sensors[0].display_name = "Sensor 1";
  config.sensors[0].temperature_offset = 0.0;
  config.sensors[0].temperature_scale = 1.0;
  config.sensors[0].enabled = true;
  
  // Initialize sensor 2 (index 1)
  config.sensors[1].display_name = "Sensor 2";
  ...
  Serial.println("📊 Sensor 1: " + config.sensors[0].display_name);
  Serial.println("📊 Sensor 2: " + config.sensors[1].display_name);
}
